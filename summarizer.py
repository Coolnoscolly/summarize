...
class OllamaSummarizer:
    ...
    def __init__(self, cfg: OllamaConfig = None):
        ...
        try:
            ...
            if model_names and self.cfg.model not in model_names:
                print(f"Предупреждение: Модель {self.cfg.model} не найдена в списке доступных.")
    ...
    def summarize(self, text: str, is_final: bool = False, is_initial: bool = True) -> str:
        """Суммаризирует текст с использованием Ollama"""
        prompt = self._build_prompt(text, is_final, is_initial)
        try:
    ...
    def _build_prompt(self, text: str, is_final: bool = False, is_initial: bool = True) -> str:
        ...
        if is_final:
            ...
            return f"{settings.SYSTEM_PROMPT}\n\n{settings.PASS_THROUGH_MERGE_PROMPT}\n\nФрагменты для объединения:\n{text}\n\nОбъединённый документ:"