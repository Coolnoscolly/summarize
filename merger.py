...
class HierarchicalMerger:
    ...
    def merge_documents(self, documents: List[str]) -> str:
        ...
        while len(current_level) > 1:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_pair = {
                    executor.submit(self._merge_pair, pair): pair for pair in pairs
                }
    ...
    def _save_intermediate(self, summaries: List[str], filename: str):
        """Сохраняет промежуточные суммары в один файл"""
        combined = "\n\n---\n\n".join(summaries)
        output_path = os.path.join(os.path.dirname(settings.OUTPUT_FILE), filename)
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(combined)
        print(f"Промежуточный результат сохранен в: {output_path}")
